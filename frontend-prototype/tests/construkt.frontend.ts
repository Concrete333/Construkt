...
function dateProgress(startDate: string, endDate: string, currentDate: Date = new Date()): number {
...
interface Project {
  ...
  name: string;
  packages: Package[];
  milestones: Array<{ id: string; name: string; targetDate: string; status: string; packageIds: string[] }>;
...
function getProjectTotals(project: Project) {
  ...
  return { contractValue, escrowFunded, totalReleased, remaining: contractValue - totalReleased };
}
...
function buildBespokeTimeline(project: Project) {
  return project.packages.map((pkg) => ({
    ...
    status:
      pkg.status === 'Locked' ? 'blocked'
      : pkg.status === 'Released' ? 'complete'
      : pkg.requests.length ? 'in-progress'
      : 'upcoming',
  }));
}
...
describe('dateProgress', () => {
  ...
  it('returns 0 when end <= start', () => {
    expect(dateProgress('2026-06-01', '2026-01-01', new Date('2026-03-01'))).to.equal(0);
  });
...
describe('timelineStatusClass', () => {
  it('returns correct node classes', () => {
    expect(timelineStatusClass('complete')).to.equal('project-phase-timeline__node--complete');
    expect(timelineStatusClass('in-progress')).to.equal('project-phase-timeline__node--current');
    expect(timelineStatusClass('blocked')).to.equal('project-phase-timeline__node--complete');
    expect(timelineStatusClass('upcoming')).to.equal('project-phase-timeline__node--future');
  });

  it('defaults to future for unknown status', () => {
    expect(timelineStatusClass('')).to.equal('project-phase-timeline__node--future');
    expect(timelineStatusClass('idle')).to.equal('project-phase-timeline__node--future');
  });
});
...
describe('getProjectTotals', () => {
  const project: Project = {
    ...
    auditLog: [],
    packages: [
      { id: 'wp-1', name: 'A', cap: 100_000, funded: 80_000, released: 20_000, status: 'Funded', contractor: 'Alice', requests: [] },
      { id: 'wp-2', name: 'B', cap: 200_000, funded: 150_000, released: 50_000, status: 'Funded', contractor: 'Bob', requests: [] },
      { id: 'wp-3', name: 'C', cap: 50_000, funded: 0, released: 0, status: 'Pending', contractor: 'Alice', requests: [] },
    ],
  };
...
describe('hasAssignedContractor', () => {
  const base = { id: 'wp-1', name: 'A', cap: 100_000, funded: 0, released: 0, status: 'Pending', requests: [] };
  ...
  it('returns false for Unassigned estimate', () => {
    expect(hasAssignedContractor({ ...base, contractor: 'Unassigned estimate' })).to.be.false;
  });

  it('returns false for Unassigned', () => {
    expect(hasAssignedContractor({ ...base, contractor: 'Unassigned' })).to.be.false;
  });
...
describe('financeApprovalStatus', () => {
  const base = { id: 'wp-1', name: 'A', cap: 100_000, funded: 0, released: 0, status: 'Pending', contractor: '', requests: [] };

  it('returns explicit financeApprovalStatus when set', () => {
    expect(financeApprovalStatus({ ...base, funded: 50_000, financeApprovalStatus: 'Approved' })).to.equal('Approved');
  });

  it('returns Escrow Locked when funded > 0 and no override', () => {
    expect(financeApprovalStatus({ ...base, funded: 1 })).to.equal('Escrow Locked');
  });

  it('returns Awaiting Finance Approval when contractor set and cap > 0', () => {
    expect(financeApprovalStatus({ ...base, contractor: 'Alice', cap: 100_000 })).to.equal('Awaiting Finance Approval');
  });

  it('returns Estimate when no contractor and no funding', () => {
    expect(financeApprovalStatus({ ...base, contractor: '' })).to.equal('Estimate');
  });

  it('returns Estimate when contractor is Unassigned', () => {
    expect(financeApprovalStatus({ ...base, contractor: 'Unassigned' })).to.equal('Estimate');
  });
});
...
describe('buildBespokeTimeline', () => {
  const project: Project = {
    ...
    auditLog: [],
    packages: [
      { id: 'wp-1', name: 'Locked Package', cap: 100_000, funded: 0, released: 0, status: 'Locked', contractor: 'Alice', requests: [] },
      { id: 'wp-2', name: 'Released Package', cap: 100_000, funded: 100_000, released: 100_000, status: 'Released', contractor: 'Bob', requests: [{ id: 'r-1', amount: 100_000, status: 'Released' }] },
      { id: 'wp-3', name: 'Active Package', cap: 100_000, funded: 50_000, released: 0, status: 'Funded', contractor: 'Carol', requests: [{ id: 'r-2', amount: 50_000, status: 'Submitted' }] },
      { id: 'wp-4', name: 'Quiet Package', cap: 50_000, funded: 0, released: 0, status: 'Pending', contractor: 'Dave', requests: [] },
    ],
  };
